...
Responda APENAS com o objeto JSON puramente, sem formatação markdown de código (como \`\`\`json ... \`\`\`).`;
        ...
        if (isQuotaError(resData) && primaryModel !== 'gemini-1.5-flash-8b') {
            console.warn(`[GEMINI API WARNING] Limite excedido para o modelo ${primaryModel}. Acionando fallback para gemini-1.5-flash-8b...`);
            resData = await tryCallGemini('gemini-1.5-flash-8b');
        }
